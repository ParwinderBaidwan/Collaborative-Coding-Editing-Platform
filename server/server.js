...
io.on('connection', function (socket) {
  ...
  // New code: Handle when a user starts editing
  socket.on('start editing', ({ roomId, username }) => {
    socket.to(roomId).emit('user editing', { username });
  });

  // New code: Handle when a user stops editing
  socket.on('stop editing', ({ roomId }) => {
